...
export default function PLSongCard({ code, playlist, cover = "https://lastfm.freetls.fastly.net/i/u/500x500/42851f3f78390ec7f5bacd31c761c681.jpg", song = "Canción", artist = "Artista", duration = "3:33", onClick, getData }) {
    ...
    const deleteData = async () => {
        try {
            ...
            if (response) {
                ...
                getData();
                setDeleteError("La canción fue borrada");
            }
        } catch (error) {
            console.error('Error al obtener datos de la API:', error);
            setDeleteError("La canción no pudo eliminarse");
        }
    };
    ...
    return (
        <div className="card flex justify-center md:pr-10 pr-6 bg-light-blue text-sm md:text-base w-full lg:w-11/12 h-20">
            {deleteError && (
